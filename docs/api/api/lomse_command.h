...
public:
    ...
    */
    ...
    /*
        This enum describes the available policies for updating the SelectionSet after executing
        a command
    */
    /*
    ...
    */
    ...
    /**
        ...
        the command, the name could have not been set and the returned value could be empty or
        could be an incomplete string.
    */
    inline std::string get_name() { return m_name; }

    /*
        Returns @true if the command is reversible, that is, if the command supports undo/redo (e.g. insert object),
        and @false if it does not support undo/redo (e.g. print)
    */
    ...
    /*
        ...
        Command sequences can be recorded and replayed at other point or on another document, allowing the creation
        of <i>macros</i> or <i>scripts</i>. But not all commands are suitable to be recorded and replayed. 
        Recordable commands are those that can be recorded and replayed safely. All reversible commands are
        recordable by nature, as this is required for supporting redo. See is_reversible().
    */
    //inline bool is_recordable() { return m_flags &  k_recordable; }

    /**
        Returns an error message with the error explanation. This method should be invoked after executing
        a command that fails. Otherwise it will return an empty string.
    */
...
{
public:
    // These constructors are defined but do not operate on selections but on cursor pointed object
...
public:
    //
        ...
        <b>Remarks</b>
        - The command will return a failure code if the selection is empty or the selected notes can not 
            be on the same beam (e.g. notes in different voices):
        - After executing the command:
            - the selection will not be altered.
            - the cursor will not change its position.
...
public:
    ...
    /**
        This enum describes the posible actions for CmdSelection commands.
    */
    ...
    */
    ...
    /**
        ...
        @param cmd The type of action to do on the selection set. Must be a value from enum #ESelectionAction.
        @param id The ID of the object to add to or remove from the selection set.
        @param name The displayable name for the command. If not specified or empty will be replaced
            by one of the following strings, depending on value for parameter <i>cmd</i>:
