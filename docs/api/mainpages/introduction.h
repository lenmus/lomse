...
- The View: It is an observer of the Document, and its main responsibility is to render it. The rendering type depends on the specific view class used. For instance, class <i>GraphicView</i> renders the document on a bitmap. Other classes would be possible (i.e. a view class to render the document as an SVG stream, a view for rendering as Braille code, a view for rendering as source code, etc.) but currently Lomse only has implemented several variations of <i>GraphicView</i> (<i>VerticalBookView</i>, <i>HorizontalBookView</i>, and <i>SimpleView</i> ). The Lomse MVC model supports having many simultaneous views for a document.  The Interactor is the owner of the View, and there is an Interactor per View.
